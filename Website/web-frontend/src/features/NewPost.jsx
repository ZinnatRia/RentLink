...
const NewPost = () => {
  ...
  function getDescription(isBangla) {
    const q = isBangla
      ? "Apnar baaasha shomporke kichchu boolun"
  ...
  useEffect(() => {
    ...
    Object.keys(files).forEach((key) => {
      setImages((current) => [
        ...current,
        [files.item(key).name, files.item(key)],
      ]);
    });
  }, [img]);
      ...
      <div className="addressbox w-[100%]">
        ...
        {locationCoordinates.longitude === null && (
          <p className="text-[1rem]">
            Location may not appear in map. Try to pick location from search if
            it exist.
          </p>
        )}
      </div>
